...
class TestGcpEngine(unittest.TestCase):
    ...
    def test_custom_role_setiampolicy_is_privesc(self):
        ...
        self.assertEqual(policy.metadata["roleType"], "custom")
    ...
    def test_role_type_derivation(self):
        model = self.provider.scan(
            {
                "predefinedRoles": [
                ...
                "bindings": [
                    {"role": "roles/owner", "members": ["user:a@b.com"], "resource": "projects/p"},
                ],
            }
        )
        self.assertEqual(model.policies["roles/storage.admin"].metadata["roleType"], "predefined")
        # Basic roles are their own type, even when only referenced by a binding.
        self.assertEqual(model.policies["roles/owner"].metadata["roleType"], "basic")

    def test_public_member_flagged_and_listed_not_synthesized(self):
        model = self.provider.scan(
            {
                "bindings": [
        ...
        self.assertIn("allUsers", policy.attached_to["public"])
        # No synthetic principal is created for public members.
        self.assertEqual(len(model.users), 0)
        self.assertEqual(len(model.groups), 0)

    def test_every_role_has_public_list(self):
        model = self.provider.scan(
            {"customRoles": [{"name": "projects/p/roles/r", "includedPermissions": ["storage.objects.get"]}]}
        )
        self.assertEqual(model.policies["projects/p/roles/r"].attached_to["public"], [])
    ...
    def test_member_types_route_to_collections(self):
        ...
        # service accounts are identities: they land in users with a discriminator
        self.assertEqual(len(model.users), 2)  # user:a@b.com + serviceAccount:sa@...
        sa = model.get_principal(USER, "serviceAccount:sa@p.iam.gserviceaccount.com")
    ...
    def test_deleted_service_account_member(self):
        member = "deleted:serviceAccount:gone@p.iam.gserviceaccount.com?uid=123"
        model = self.provider.scan(
            {"bindings": [{"role": "roles/viewer", "members": [member], "resource": "projects/p"}]}
        )
        sa = model.get_principal(USER, member)
        self.assertIsNotNone(sa)
        self.assertEqual(sa.name, "gone@p.iam.gserviceaccount.com")
        self.assertEqual(sa.metadata.get("provider_kind"), "service_account")
        self.assertTrue(sa.metadata.get("deleted"))

    def test_unknown_member_type_is_user_with_unknown_kind(self):
        model = self.provider.scan(
            {"bindings": [{"role": "roles/viewer", "members": ["principal://iam.example/wif"], "resource": "p"}]}
        )
        principal = model.get_principal(USER, "principal://iam.example/wif")
        self.assertIsNotNone(principal)
        self.assertEqual(principal.metadata.get("provider_kind"), "unknown")
