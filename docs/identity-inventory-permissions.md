...
| `cloudtrail:LookupEvents` | `created_by` for users/roles created in the last 90 days (`CreateUser`/`CreateRole`/`CreateServiceLinkedRole` — service-linked roles are attributed from the response's role name); `CreateAccessKey`/`CreateLoginProfile` events corroborate credential shape for report-gap users | `created_by` null except access keys, whose structural attribution to the owning user needs no extra permission (`--skip-cloudtrail-events` opts out explicitly) |
...
| `Allow group <X> to read audit-events in tenancy` (365-day retention) | `created_by` via create-user / create-dynamic-group events | null — unless Identity Domains supplies `idcsCreatedBy` (next row) |
| `Allow group <X> to read domains in tenancy` plus user read on each domain | `idcsCreatedBy` merged onto users — stored on the user, so it attributes identities **older** than audit retention too | users created outside the audit window keep `created_by` null |
